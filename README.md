...
方案确认 ─► worktree ─► implement ─► 双边审查 ⟲ fix ─► pr ─► ci ⟲ fix/冲突 ─► 合并确认 ─► cleanup ─► done
          │origin/base │测试门禁      │双 claude 各自独立        │挂auto-merge       │auto-merge      │删 worktree
          │最新代码建   │              │JSON结论,代码裁决          │gh轮询,代码裁决    │或兜底squash      （成功/失败都清）
          │独立 worktree│             │第1轮全量,第2轮复审增量     │修复≤5轮熔断       │
          │             │             │终局 previous/delta 限域救援│
          └── 任一环节修复仍失败 → failed；环境问题 resume，审查熔断 supersede ──┘
```
...
- **双边审查是质量红线，且是分工制**——`reviewEngines`（默认 `claude` + `claude-fidelity`，
  即两个独立的 claude code 审查者，各自全新会话）各自独立跑审查、
  各写各的 `.ship/review-<engine>.json`，**必须两边都 pass 才算过**；任一方打回都要修复后复审，
  没有人工兜底。`reviewRoles` 给两边分工（默认 claude→architecture：全局架构视角，专抓"只为实现
  而实现"的设计短视；claude-fidelity→fidelity：把方案当合同逐条核对实现有没有走样），各扫各的盲区，
  ...
  escape_reason 才能例外阻塞）。终局轮若只剩 `previous`（旧意见未修净）或 `delta`
  （修复增量新问题），还有一次锁定 Finding 集合的限域救援：自动换用不同 engine 的全新会话修复，
  再由原打回方复核；出现 `other`（重新开荒）或救援后仍不过才熔断
- **实现前验收矩阵**——implement 前用全新会话读取方案与生产代码，明确每条验收项的真实入口、
  禁止 mock 的边界、测试层级和命令提示，再把矩阵注入实现 prompt
- **结构化修复证据**——must-fix finding 带稳定 ID、invariant、代码证据、最小复现和
  `required_test_boundary`；修复必须写 resolution manifest，harness 实际执行绑定的 test command
  后才允许复审
- **分层测试门禁**——`.ship/testplan.json` 分为 `fast / required / e2e`；仓库配置是不可被
  engine 覆盖的最低门槛，engine 只能追加。实现和审查修复产生的代码在进入 reviewer 前跑全套；
  CI 修复跑 required；review 放行后再只读复验全套，禁止此时自动改出“未审代码”
...
#    failed → 环境类问题用 resume；审查轮数熔断用 supersede 继承失败分支与 findings
...
环境/网络类失败用 `resume`，审查轮数熔断用 `supersede` · `done` PR 已自动合并。
...
新 run 优先使用 `.ship/testplan.json` 的分层门禁；`testCmd` 与 `.ship/testcmd` 只为旧 run 兼容。
仓库配置的 `testPlan` 命令总会保留，engine 只能追加，不能用一条窄测试替换团队门禁。
优先级：API 请求体 > 仓库 ship.config.json > 默认值。
review 轮上限是 2：第 1 轮全量已双边背书，第 2 轮复审收窄到「旧意见 + 修复增量」，
再多轮就是重新开荒（每轮挖新问题、不收敛，实测 4 个 run 全部跑满轮数）；
终局轮只剩 previous/delta 时另有一次不占轮次的限域救援；救援使用不同 engine 的全新会话。
CI/测试修复保持 5 轮预算。
超限后依然直接判失败，不会挂起。

`reviewEngines` 决定 autoReview 阶段哪几个 engine 参与双边审查，默认两个独立的 claude 审查者
（`claude-fidelity` 是指向 claude SDK 的别名引擎——审查产物/会话/事件按 engine 名分桶，
两个审查者必须不同名）；改成 `["claude", "codex"]` 可换回跨厂商组合，`["claude"]` 退回单边审查，
也可以填第三个自定义 engine 名做三边审查。
...
2. **按步骤路由不同引擎**（`stageEngines`）：给验收预检/实现/修复/CI修复/解冲突这些步骤单独指定 engine，
   不配则用默认 `engine`：
   ...
   可路由的步骤：`preflight` / `implement` / `fix` / `testFix` / `ciFix` / `conflict`。
...
- 侧边栏四分区：进行中 / 需要处理（failed）/ 已完成（可一键归档）/ 已归档（折叠懒加载），区内按最近活动排序。
- run 详情默认「进度」tab：React Flow 流水线图（审查双引擎并行节点、修复/CI 循环回边、依赖等待节点），
  点节点看该步骤的输出尾部与错误；全量日志在「日志」tab 兜底。SSE 支持 `?filter=core` 不推 engine 逐行输出。
- 组视图：泳道 DAG（每仓库一条泳道，依赖边从上游「完成」连到下游「等待上游」）+ 成员表格（依赖列、迷你进度条）。

## 仓库间依赖（run group）

组清单 repos 项可声明 `name` / `dependsOn` / `publishes`（见 skills/devflow/SKILL.md 的多仓库步骤）：
下游 implement 后进入 `awaitDeps` 阶段，等上游全部 done 且发布探测通过（默认 `npm view <pkg> version`
对比组创建时的基线版本，`publishes.check` 可自定义，超时默认 30 分钟），随后自动 depBump
（依赖更新到探测版本 + 修适配 + 过测试门禁）再进入双边审查。依赖成环/未知 name 整组拒绝创建。

## 代码结构

```
lib/        核心（与框架无关）：types / store(落盘+事件流) / pipeline(状态机) / exec / prompts /
            runtime(单例) / sidebar(分区排序) / progressGraph(进度图推导) / deps(依赖校验与等待决策)
app/        Next.js：page.tsx(只读看板) + ProgressFlow/GroupFlow(React Flow 视图) + api/runs|groups/**(REST + SSE)
cli/        CLI：serve / start / ls / groups / status / attach / resume / archive
```
